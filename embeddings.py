"""
embeddings.py - HuggingFace API Client for Embeddings
...
_hf_client = InferenceClient(token=HF_API_KEY, timeout=60.0) if HF_API_KEY else None
...
def _get_embeddings_api(texts: List[str]) -> np.ndarray:
    ...
    for i in range(0, len(safe_texts), BATCH_SIZE):
        ...
        for attempt in range(3):
            try:
                # Use official huggingface_hub feature_extraction bindings
                embeddings_raw = _hf_client.feature_extraction(
                    text=batch_texts, 
                    model="sentence-transformers/all-MiniLM-L6-v2",
                )
                
                # InferenceClient returns raw numbers (usually nested arrays)
                batch_embeddings = np.array(embeddings_raw)
                ...
                raise RuntimeError(f"HF API 400 Bad Request. Try reducing BATCH_SIZE. Detail: {e}")
                
            except HfHubHTTPError as e:
                if e.response.status_code == 503 and attempt < 2:
                    print("HF Model loading/503, waiting 15s...")
                    time.sleep(15)
                    continue
                if attempt == 2:
